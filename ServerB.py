...
class ServerB(Socket):
    ...
    def resolve_CS_REQ(self, cs_req):
        """ 解密 Ticket，解密 enc_data，对比 UserA，对比 timestamp, 完成密钥协商 """
        ...
        UserA_2, timestamp = dec_data.split(SIGN)

        timestamp_now = set_timestamp()

    def set_CS_REP(self, cs_req):
        """ CS_REP = { timestamp } Kclt-srv, pub_key_b """
        ...
        CS_REP = self.crypt_sm4_kclt_srv.crypt_ecb(timestamp) + SIGN + self.pub_key_b.encode()
